...
{	
	...
	function FeuilleListeMatchs()
	{
		...
		if ($idEvenement != -1)
		{
			$titreEvenementCompet = 'Event : '.$myBdd->GetEvenementLibelle($idEvenement);
		}
		else
		{
			$arrayCompetition = $myBdd->GetCompetition($codeCompet, $codeSaison);
			$titreEvenementCompet = 'Competition : '.$arrayCompetition['Libelle'].' ('.$codeCompet.')';
		}
		...
		else
		{
			if ($dateDebut == $dateFin)
				$titreDate = utyDateFrToUs($dateDebut);
			else
				$titreDate = utyDateFrToUs($dateDebut)." to ".utyDateFrToUs($dateFin);
		}
		...
		$pdf->Cell(273,6,"Game list",'LRB','1','C');
		$pdf->Ln(7);
			...
			elseif ($EquipesAffectAuto[3] != '')
				...
				if ($rupture != $Oldrupture)
				{
					...
					default :
						...
						$pdf->Cell(150, 5, date_format($rupture2, 'l jS F Y').' - '.$row['Lieu'], 'LTBR','1','C');
						$pdf->Cell(8,5, '#','LTRB','0','R');
						$pdf->Cell(10,5, 'Time','TRB','0','C');
						$pdf->Cell(17,5, 'Compet.','TRB','0','C');
						if ($PhaseLibelle == 1)
