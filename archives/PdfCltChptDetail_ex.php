	...
	function FeuilleCltNiveau()
	{
	  MyPage::MyPage();
		...
		for ($i=0;$i<$num_results;$i++)
		{
				...
				$pdf->Cell(10, 5, $row['Clt_publi'], 'LTBR','0','R');
				$pdf->Cell(45, 5, $row['Libelle'],'LTBR','0','L');
				$pdf->Cell(15, 5, $pts,'LTBR','0','C');
				$pdf->Cell(15, 5, $row['J_publi'],'LTBR','0','C');
				$pdf->Cell(15, 5, $row['G_publi'],'LTBR','0','C');
				$pdf->Cell(15, 5, $row['N_publi'],'LTBR','0','C');
				$pdf->Cell(15, 5, $row['P_publi'],'LTBR','0','C');
				$pdf->Cell(15, 5, $row['F_publi'],'LTBR','0','C');
				$pdf->Cell(15, 5, $row['Plus_publi'],'LTBR','0','C');
				$pdf->Cell(15, 5, $row['Moins_publi'],'LTBR','0','C');
				$pdf->Cell(15, 5, $row['Diff_publi'],'LTBR','1','C');
				
				$sql  = "Select a.Id_equipeA, a.ScoreA, c.Libelle LibelleA, ";
				$sql .= "       a.Id_equipeB, a.ScoreB, d.Libelle LibelleB, ";
				$sql .= " a.Id, a.Id_journee, a.Date_match, a.Heure_match, a.Terrain, b.Lieu, a.Validation, a.Publication ";
				...
				for ($j=0;$j<$num_results2;$j++)
				{
						...
						if ( ($row2['ScoreA'] == '') || ($row2['ScoreA'] == '?') || ($row2['Validation'] != 'O') )
							continue; // Score non valide ...
						
						if ( ($row2['ScoreB'] == '') || ($row2['ScoreB'] == '?') || ($row2['Validation'] != 'O') )
							continue; // Score non valide ...
