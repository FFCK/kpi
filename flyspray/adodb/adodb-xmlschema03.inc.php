	...
	*/
	function xslt_error_handler( $parser, $errno, $level, $fields ) {
		...
		foreach( $msg as $label => $details ) {
			$error_details .= '<tr><td><b>' . $label . ': </b></td><td>' . htmlspecialchars( $details ) . '</td></tr>' . "\n";
		}
	...
	function ExtractSchema( $data = FALSE, $indent = '  ', $prefix = '' , $stripprefix=false) {
		...
		if( is_array( $tables = $this->db->MetaTables( 'TABLES' , ($prefix) ? $prefix.'%' : '') ) ) {
			foreach( $tables as $table ) {
				...
				$schema .= $indent . '<table name="' . htmlspecialchars( $table ) . '">' . "\n";
				...
				if( is_array( $fields ) ) {
					foreach( $fields as $details ) {
						...
						if( isset($details->has_default) && $details->has_default ) {
							$content[] = '<DEFAULT value="' . htmlspecialchars( $details->default_value ) . '"/>';
						}
						...
						$schema .= str_repeat( $indent, 2 ) . '<field name="' . htmlspecialchars( $details->name ) . '" type="' . $type . '"' . $extra;
				...
				if( is_array( $indexes ) ) {
					foreach( $indexes as $index => $details ) {
						...
						foreach( $details['columns'] as $column ) {
							$schema .= str_repeat( $indent, 3 ) . '<col>' . htmlspecialchars( $column ) . '</col>' . "\n";
						}
	...
	*/
	function getSQL( $format = NULL, $sqlArray = NULL ) {
		...
		switch( strtolower( $format ) ) {
			...
			case'html':
				return !empty( $sqlArray ) ? nl2br( htmlspecialchars( implode( ";\n\n", $sqlArray ) . ';' ) ) : '';
		}
