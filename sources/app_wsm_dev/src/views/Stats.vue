    ...
    <div v-if="network && game" class="container-fluid mt-1">
      ...
      <div class="row mt-1 gx-1">
        ...
        <div class="col border text-center p-1" :style="'color: ' + game.equipe2.colortext + '; background-color: ' + game.equipe2.color1 +';'">
          <div class="row g-1">
            <span class="col-md-2 col-sm-4 col-xs-6" v-for="(player, index) in team2" :key="index">
              <button
                :class="{
                  btn: true,
                  'btn-lg': true,
      ...
      <div class="row mt-1 g-1">
        ...
        <div class="d-grid col-3">
          <button
            ...
            @click="btnMode = 4">
            {{ $t("Shot") }} <i class="bi bi-box-arrow-in-up"></i>
          </button>
        </div>
        <div class="d-grid col-3">
          <button
            ...
            :class="{
              btn: true,
              'btn-lg': true,
              'btn-outline-info': btnMode !== 5,
              'btn-info': btnMode === 5
            }"
            :disabled="btnMode > 0 && btnMode < 4"
            @click="btnMode = 5">
            {{ $t("Shot") }} <i class="bi bi-box-arrow-up-right"></i>
          </button>
        </div>
        <div class="d-grid col-3">
          <button
...
export default {
  ...
  methods: {
    ...
    async fetchGame () {
      ...
      if (this.gameTarget !== this.game?.id_match) {
        try {
    ...
    gameProcess (game, score) {
      ...
      this.matchPeriodFormated = score.periode.substr(1)
    },
    async fetchNetwork () {
    ...
    action (team, player) {
      switch (this.btnMode) {
        ...
        case 5:
        case 7:
          this.submit(this.game['equipe' + team].id, player, 'shot-out')
          this.btnMode = 0
          break
      }
    },
    submit (team, player, action) {
      if (this.matchHorlogeStarted) {
        const obj = {
          user: this.user,
