			...
			<form method="POST" action="GestionParamJournee.php" name="formParamJournee" enctype="multipart/form-data">
				...
				<div class='blocRight Right3'>
					<table class='tableau2'>
						...
						{if $profile <= 2}
							<tr>
								<td><label for="J_saison">{#Saison#}</label>
									<img hspace="2" width="18" height="18" src="../img/danger.png" alt="Attention" title="Attention"
										border="0">
									<select name="J_saison" onchange="alert('Attention')">
										{section name=i loop=$arraySaisons}
											<Option Value="{$arraySaisons[i].Code}" {if $arraySaisons[i].Code == $J_saison}selected{/if}>
						...
						{else}
							<tr>
								<td><label for="J_saison">{#Saison#} : </label>{$J_saison}<input type='hidden' name='J_saison'
										Value='{$J_saison}' /></td>
								<td colspan=3><label for="J_competition">{#Competition#} : </label>{$J_competition}<input type='hidden'
										name='J_competition' Value='{$J_competition}' /></td>
							</tr>
						{/if}
						<tr>
							<td width="50%">
								...
								<select id="PhaseList" name="PhaseList" onChange="Phase.value=this.options[this.selectedIndex].value">
									...
									<optgroup label="Models (EN)">
										...
										<Option Value="Eighth finals">Eighth finals</Option>
							...
							<td width="17%">
								...
								<img width="12" src="../img/b_help.png" {if $lang == "en"} title="Level : Importance of the phase in the géléral classification. For example :
							...
							<td width="16%">
								...
								<img width="12" src="../img/b_help.png" {if $lang == "en"} title="Team count (for classifying groups)"
									{else} title="Nombre d'équipes (pour les poules de classement)" 
									{/if}>
						...
						<tr>
							<td><label for="AvecMatchs">{#Inclure_matchs#}</label><input type="checkbox" name="AvecMatchs" value="oui">
							</td>
							<td colspan=3><label for="CodMatchs">{#Encoder_matchs_de_poule#}</label><input type="checkbox"
									name="CodMatchs" value="oui"></td>
						</tr>
						<tr>
							<td colspan=4>
								<input type="button" onclick="Duppli();" name="Dupliquer"
									value="{#Sauvegarder_comme_nouvelle_journee#} ({#Dupliquer#})">
								<hr>
							</td>
						</tr>
					</table>
						...
						<table class='tableau2'>
							...
							<tr>
								<td colspan=2>
									...
									&nbsp;
									<a href="#" onclick="setCheckboxes('formParamJournee', 'checkListJournees', true);return false;"><img
											hspace="2" width="21" height="19" src="../img/tous.gif" alt="Sélectionner tous"
											title="Sélectionner tous" border="0"></a>
									&nbsp;
									<a href="#" onclick="setCheckboxes('formParamJournee', 'checkListJournees', false);return false;"><img
											hspace="2" width="21" height="19" src="../img/aucun.gif" alt="Sélectionner aucun"
											title="Sélectionner aucun" border="0"></a>
									<br>
								</td>
							</tr>
							...
							<tr>
								<td colspan=2>
									<input type="button" onclick="DuppliListJournees()" name="Duppliquer" value="{#Appliquer#}"
										title="{#Enregistrer_avant_appliquer#} !">
									<hr>
								</td>
							</tr>
						</table>
