					...
					<tr class='filtres cadregris'>
							...
							<select name="evenement" id="evenement" onChange="submit();">
								{section name=i loop=$arrayEvenement}
									...
									<Option Value="{$arrayEvenement[i].Id}" {$arrayEvenement[i].Selection}>
										{$smarty.config.$evt_libelle|default:$evt_libelle}</Option>
								{/section}
							</select>
						</td>
						<td style="text-align: center">
							...
							{if $profile <= 6 && $AuthModif == 'O'}
								<a href="#" id="InitTitulaireCompet" title="{#InitTitulaireCompet#}"><img height="22"
										src="../img/b_update.png"></a>
							{/if}
							<br>
										...
										{section name=j loop=$options}
											...
											{if $options[j].Code == '*'}
												<Option Value="{$options[j].Code}" {$options[j].selected}>{$options[j].Code} -
													{#Toutes_les_competitions_de_l_evenement#}</Option>
											{else}
												<Option Value="{$options[j].Code}" {$options[j].selected}>{$options[j].Code} -
							...
							<select name="filtreTour" id="filtreTour" onChange="submit();">
								...
								{section name=foo start=1 loop=6}
									<Option Value="{$smarty.section.foo.index}"
										{if $filtreTour == $smarty.section.foo.index}selected{/if}>{#Tour#}
							...
							<select name="orderMatchs" onChange="ChangeOrderMatchs('{$idSelJournee}');">
								{section name=i loop=$arrayOrderMatchs}
									...
									{if $orderMatchs eq $arrayOrderMatchs[i].Key}
										<Option Value="{$arrayOrderMatchs[i].Key}" Selected>{$smarty.config.$currentOrderMatch}
										</Option>
									{else}
						...
						<td style="text-align: center">
							...
							<input type='checkbox' name='filterAtt' id='filterAtt' value='on'
								{if $filtreMatchsNonVerrouilles == 'on'}checked{/if}>
						</td>
					</tr>
				</table>
				{if $profile <= 6 && $AuthModif == 'O'}
					<div class='blocTop'>
						<table id="formMatch">
							<tr class="hideTr">
								<td style="text-align: left" title="{#Intervale_matchs_title#}">
								...
								<td>
									Type :
									<img id="typeMatch1" src="../img/type{$Type|default:'C'}.png" {if $Type == "E"}
										alt="{#Elimination#}" title="{#Match_eliminatoire#}" {else} alt="{#Classement#}"
										title="{#Match_de_classement#}" {/if} height="23">
								...
								<td style="text-align: left">
									<table>
										<tr>
											<td>
												<label for="Libelle">{#Intitule_codage#}</label> <img src="../img/information.gif"
													width="14" title="<img src='../img/codage.png' width='250'>">
												<br>
												<input type="text" size="18" name="Libelle" placeholder="{#Code_Exemple#}"
													value="{$Libelle}" maxlength="30" tabindex="7">
											</td>
											<td>
												<label for="Num_match">{#Match#} N°</label>
												<br>
												<input type="text" size="3" name="Num_match" id="Num_match" value="{$Num_match}"
													tabindex="5" />
											</td>
										</tr>
									</table>
								...
								<td style="text-align: left">
									...
									<a href="#" id="InitTitulaireEquipeA" title="{#InitTitulaireEquipe#}">
										<img height="22" src="../img/b_update.png">
									</a>
									<br>
									<select name="equipeA" id="equipeA" onChange="changeEquipeA();" tabindex="8">
										...
										{section name=i loop=$arrayEquipeA}
											<Option Value="{$arrayEquipeA[i].Id}" {$arrayEquipeA[i].Selection}>
												...
												{if $arrayEquipeA[i].Poule != ''}
													{$arrayEquipeA[i].Poule|string_format:" (%s)"|default:""}
												{/if}
											</Option>
										{/section}
									</select>
								...
								<td style="text-align: left">
									...
									<input type="text" size="30" name="arbitre1" id="arbitre1" placeholder="{#ref_placeholder#}"
										value="{$arbitre1}" tabindex="12" />
									<input type="text" size="5" name="arbitre1_matric" readonly id="arbitre1_matric"
										value="{$arbitre1_matric}" />
									<br />
									<label for="comboarbitre1">{#Principal#}</label>
									<select class="combolong" name="comboarbitre1"
									...
									<select class="combolong" name="comboarbitre1b" id="comboarbitre1b" tabindex="13">
										...
										{section name=i loop=$arrayArbitreEquipes}
											<Option Value="{$arrayArbitreEquipes[i].Matric}">{$arrayArbitreEquipes[i].Identite}
											</Option>
										{/section}
									</select>
							...
							<tr class="hideTr">
								<td style="text-align: center" colspan="2">
									...
									<a href="#" id="InitTitulaireJournee" title="{#InitTitulaireJournee#}">
										<img height="22" src="../img/b_update.png">
									</a>
									<br>
									...
									<table>
										<tr>
											<td>
												...
												<br>
												<input type="text" size="10" class='date' name="Date_match" value="{$Date_match}"
													tabindex="3" onfocus="displayCalendar(document.forms[0].Date_match, 
											...
											<td>
												...
												<br>
												<input type="text" size="5" class='champsHeure' name="Heure_match"
													value="{$Heure_match}" tabindex="4" />
											</td>
											<td>
												<label for="Terrain">{#Terrain#}</label>
												<br>
												<input type="text" size="3" name="Terrain" value="{$Terrain}" maxlength=12
													tabindex="6" />
											</td>
										</tr>
									</table>
								...
								<td>
									...
									<a href="#" id="InitTitulaireEquipeB" title="{#InitTitulaireEquipe#}"><img height="22"
											src="../img/b_update.png"></a>
									<br>
									<select name="equipeB" id="equipeB" onChange="changeEquipeB();" tabindex="10">
										...
										{section name=i loop=$arrayEquipeB}
											<Option Value="{$arrayEquipeB[i].Id}" {$arrayEquipeB[i].Selection}>
												...
												{if $arrayEquipeB[i].Poule != ''}
													{$arrayEquipeB[i].Poule|string_format:" (%s)"|default:""}
												{/if}
											</Option>
										{/section}
									</select>
								...
								<td>
									...
									<input type="text" size="30" name="arbitre2" id="arbitre2" placeholder="{#ref_placeholder#}"
										value="{$arbitre2}" tabindex="14" />
									<input type="text" size="5" name="arbitre2_matric" readonly id="arbitre2_matric"
										value="{$arbitre2_matric}" />
									<br />
									<label for="comboarbitre2">{#Secondaire#}</label>
									<select class="combolong" name="comboarbitre2"
										onChange="arbitre2_matric.value=this.options[this.selectedIndex].value; arbitre2.value=this.options[this.selectedIndex].text;"
										tabindex="15">
										<Option Value="-1"></Option>
										{section name=i loop=$arrayArbitre}
											<Option Value="{$arrayArbitre[i].Matric}">{$arrayArbitre[i].Identite}</Option>
							...
							<tr class="hideTr">
								...
								<td style="text-align: center" colspan=2>
									<input type="button" onclick="Add();" id="addMatch" name="addMatch" value="{#Ajouter#}"
										tabindex="16">
									<input type="button" {if $idMatch eq '-1'} disabled {/if} onclick="Update();" id="updateMatch"
										name="updateMatch" value="{#Modifier#}" tabindex="17">
									<input type="button" onclick="Raz();" id="razMatch" name="razMatch" value="{#Annuler#}"
										tabindex="18">
								</td>
							...
							<tr id='clickdown'>
								<td colspan="6" style="text-align: left; color:#555555"><i><u>{#Afficher_le_formulaire#}</u></i>
								</td>
							</tr>
						</table>
					</div>
						...
						<tr>
							<td width="490">
								<fieldset>
									...
									<a href="#" {$TropDeMatchs}
										onclick="setCheckboxes('formJournee', 'checkMatch', false);return false;"><img
									...
									{if $profile <= 6 && $AuthModif == 'O'}
										<a href="#" {$TropDeMatchs} onclick="RemoveCheckboxes('formJournee', 'checkMatch')"
											title="{#Supprimer#} {$TropDeMatchsMsg}"><img height="25"
												src="../img/glyphicons-17-bin.png" /></a>
										<a href="#" {$TropDeMatchs}
											onclick="SelectedCheckboxes('formJournee', 'checkMatch');publiMultiMatchs();"
										...
										{if $profile <= 4 && $AuthModif == 'O'}
											<a href="#" {$TropDeMatchs}
												...
												title="{#Verrouiller_Publier#} {$TropDeMatchsMsg}"><img height="25"
													src="../img/oeilverrou2.gif" /></a>
											<a href="#" {$TropDeMatchs}
												onclick="SelectedCheckboxes('formJournee', 'checkMatch');verrouMultiMatchs();"
												title="{#Verrouiller#} {$TropDeMatchsMsg}"><img height="25"
													src="../img/verrou2.gif" /></a>
										{/if}
										<a href="#" {$TropDeMatchs}
											onclick="SelectedCheckboxes('formJournee', 'checkMatch');affectMultiMatchs();"
											title="{#Affectation_auto#} {$TropDeMatchsMsg}"><img height="25"
												src="../img/AffectAuto.gif" /></a>
										<a href="#" {$TropDeMatchs}
											onclick="SelectedCheckboxes('formJournee', 'checkMatch');annulMultiMatchs();"
											title="{#Annuler_Affectation_auto#} {$TropDeMatchsMsg}"><img height="25"
												src="../img/AnnulAuto.gif" /></a>
										<a href="#" {$TropDeMatchs}
											onclick="SelectedCheckboxes('formJournee', 'checkMatch');changeMultiMatchs();"
											title="{#Changer_de_poule#} {$TropDeMatchsMsg}"><img height="25"
												src="../img/Chang.gif"></a>
									{/if}
									<a href="#" {$TropDeMatchs}
								...
								<fieldset>
									...
									<a href="FeuilleListeMatchs.php" {$TropDeMatchs} Target="_blank"
										title="Liste des Matchs {$TropDeMatchsMsg}"><img height="25"
											src="../img/ListeFR.gif" /></a>
									&nbsp;
									<a href="FeuilleListeMatchsEN.php" {$TropDeMatchs} Target="_blank"
										title="Game list (EN) {$TropDeMatchsMsg}"><img height="25"
											src="../img/ListeEN.gif" /></a>
									&nbsp;
									<a href="FeuilleMatchMulti.php?listMatch={$listMatch}" {$TropDeMatchs} Target="_blank"
										title="{#Feuilles_marque#} {$TropDeMatchsMsg}"><img height="25"
											src="../img/pdf2.png" /></a>
									&nbsp;
									<a href="tableau_openspout.php" title="Export (ODS)"><img height="25"
											src="../img/ods.png" /></a>
									&nbsp;
									<a href="../PdfListeMatchs.php" {$TropDeMatchs} Target="_blank"
										title="Liste publique des Matchs {$TropDeMatchsMsg}"><img height="25"
											src="../img/ListeFR.gif" /></a>
									&nbsp;
									<a href="../PdfListeMatchsEN.php" {$TropDeMatchs} Target="_blank"
										title="Public Game list (EN) {$TropDeMatchsMsg}"><img height="25"
											src="../img/ListeEN.gif" /></a>
									{if $profile <= 2}
										&nbsp;
							...
							<td>
								&nbsp;&nbsp;
								<span id='reachspan'><i>{#Surligner#}:</i></span><input type='text' name='reach' id='reach'
									size='5'>
								{if $profile <= 2}
									<span id='reachspan2'><i>{#Surligner#}:</i></span><input type='text' name='reach2' id='reach2'
										size='5'>
								{/if}
							</td>
						</tr>
					...
					<div class='blocTable' id='blocMatchs'>
						<table class='tableau' id='tableMatchs'>
							<thead>
								<tr>
									<th>&nbsp;</th>
									<th><img height="25" src="../img/oeil2.gif" title="{#Publier#} ?"></th>
									<th>{#Num#}</th>
									<th width=45>&nbsp;</th>
									<th>{#Heure#}</th>
									<th>Cat.</th>
									...
									<th>{#Equipe#} A</th>
									<th>{#Sc#} A</th>
									<th><img height="25" src="../img/verrou2.gif" title="{#Verrouiller#} ?"></th>
									<th>{#Sc#} B</th>
									<th>{#Equipe#} B</th>
									<th>{#Arbitre#} 1 </th>
									<th>{#Arbitre#} 2 </th>
							...
							<tbody>
								{section name=i loop=$arrayMatchs}
									<tr class='{cycle values="impair,pair"} {$arrayMatchs[i].StdOrSelected}'>
										<td>
											<input type="checkbox" name="checkMatch" value="{$arrayMatchs[i].Id}" id="checkDelete{$smarty.section.i.iteration}">
										</td>
										{if $arrayMatchs[i].MatchAutorisation == 'O' && $profile <= 6 && $AuthModif == 'O'}
											<td class='color{$arrayMatchs[i].Publication}2'>
												<img class="publiMatch" data-valeur="{$arrayMatchs[i].Publication}"
													data-id="{$arrayMatchs[i].Id}" height="25"
													src="../img/oeil2{$arrayMatchs[i].Publication|default:'N'}.gif"
													title="{if $arrayMatchs[i].Publication == 'O'}{#Public#}{else}{#Prive#}{/if}">
											</td>
											{if $arrayMatchs[i].Validation != 'O'}
												<td>
													<span class='directInput numMatch' Id="Numero_ordre-{$arrayMatchs[i].Id}-text"
														tabindex='1{$smarty.section.i.iteration|string_format:"%02d"}0'>
														{$arrayMatchs[i].Numero_ordre}
													</span>
												</td>
												<td width=80>
													<a href="#" class="showOn" onclick="ParamMatch({$arrayMatchs[i].Id})">
														<img height="20" src="../img/glyphicons-31-pencil.png" title="{#Editer#}">
													</a>
													<a href="FeuilleMatchMulti.php?listMatch={$arrayMatchs[i].Id}" Target="_blank">
														<img height="20" src="../img/pdf.png" title="{#Feuille_marque#} (Pdf)">
													</a>
													<br />
													<a href="#" onclick="window.open('FeuilleMarque2.php?idMatch={$arrayMatchs[i].Id}','FeuilleV2'); return false;">
														<img height="20" src="../img/glyphicons-163-ipad.png"
															title="{#Feuille_marque_en_ligne#}">
													</a>
													{if $profile <= 1}
														<a href="#" onclick="window.open('FeuilleMarque3.php?idMatch={$arrayMatchs[i].Id}','FeuilleV3'); return false;">
															<img height="20" src="../img/ipad-3.png" title="{#Feuille_marque_en_ligne#} (V3)">
														</a>
													{/if}
												</td>
												<td>
													...
													<span class='directInput heure' Id="Heure_match-{$arrayMatchs[i].Id}-time"
														tabindex="1{$smarty.section.i.iteration|string_format:'%02d'}2">
														{$arrayMatchs[i].Heure_match}
													</span>
												</td>
												<td title="{$arrayMatchs[i].Code_competition}">
													<span class="compet">
														{if $arrayMatchs[i].Soustitre2 != ''}
															{$arrayMatchs[i].Soustitre2}
														{else}
															{$arrayMatchs[i].Code_competition}
														{/if}
													</span>
												</td>
												{if $PhaseLibelle == 1}
													<td>
														<span class="directInput phase"
															tabindex='1{$smarty.section.i.iteration|string_format:"%02d"}6'
															data-match="{$arrayMatchs[i].Id}" data-idphase="{$arrayMatchs[i].Id_journee}"
															data-phase="{$arrayMatchs[i].Phase}">
															{$arrayMatchs[i].Phase|default:'&nbsp;'}
														</span>
													</td>
													<td>
														<span class='directInput text eq'
															tabindex='1{$smarty.section.i.iteration|string_format:"%02d"}3'
												...
												{else}
													...
													<td>
														<span class="lieu">
															{$arrayMatchs[i].Lieu|default:'&nbsp;'}
														</span>
													</td>
												{/if}
												<td>
													<img class="typeMatch" data-valeur="{$arrayMatchs[i].Type}"
														data-id="{$arrayMatchs[i].Id}" src="../img/type{$arrayMatchs[i].Type}.png"
														title="{if $arrayMatchs[i].Type == 'C'}{#Match_de_classement#}{else}{#Match_eliminatoire#}{/if}"
														height="23">
												</td>
												<td>
													<span class='directInput terrain'
														tabindex="1{$smarty.section.i.iteration|string_format:'%02d'}4"
														Id="Terrain-{$arrayMatchs[i].Id}-text">
														{$arrayMatchs[i].Terrain|default:'&nbsp;'}
													</span>
												</td>
												<td>
													<span class="directInput equipe{if $arrayMatchs[i].Id_equipeA < 1} undefTeam{/if}"
														...
														data-idequipe="{$arrayMatchs[i].Id_equipeA}" data-equipe="A">
														{$arrayMatchs[i].EquipeA}
													</span>
													<br />
													<a href="GestionMatchEquipeJoueur.php?idMatch={$arrayMatchs[i].Id}&codeEquipe=A"
														title="{#Composition_equipe#} A">
														<img height="20" src="../img/b_compo_match.png">
													</a>
												</td>
												<td>
													<span class='directInput score'
														tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}5"
														Id="ScoreA-{$arrayMatchs[i].Id}-text">
														{$arrayMatchs[i].ScoreA}
													</span>
												</td>
												<td class='color{$arrayMatchs[i].Validation}2'>
													{if $profile <= 6 && $AuthModif == 'O'}
														<img class="verrouMatch" data-valeur="{$arrayMatchs[i].Validation}"
															data-id="{$arrayMatchs[i].Id}" height="25"
															src="../img/verrou2{$arrayMatchs[i].Validation}.gif"
															title="{if $arrayMatchs[i].Validation == 'O'}{#Verrouille#}{else}{#Deverrouille#}{/if}">
													{else}
														<img height="25" src="../img/verrou2{$arrayMatchs[i].Validation|default:'N'}.gif"
															title="{#Verrouiller#}">
													{/if}
													{if $arrayMatchs[i].Statut == 'ON'}
														<span class="directStatutMatch statutMatchOn" data-statut="ON"
															data-id="{$arrayMatchs[i].Id}" data-periode="{$arrayMatchs[i].Periode|default:''}"
															title="{#Periode#} {$arrayMatchs[i].Periode|default:''}">
													...
													{elseif $arrayMatchs[i].Statut == 'END'}
														<span class="directStatutMatch statutMatchOn" data-statut="END"
															data-id="{$arrayMatchs[i].Id}" data-periode="{$arrayMatchs[i].Periode|default:''}"
															title="{#Match_termine#}">{$arrayMatchs[i].Statut}</span>
														<span class="scoreProvisoire"
															title="{#scoreProvisoire#}">{$arrayMatchs[i].ScoreDetailA} -
															{$arrayMatchs[i].ScoreDetailB}</span>
													{else}
														<span class="directStatutMatch scoreProvisoire" data-statut="ATT"
															data-id="{$arrayMatchs[i].Id}" data-periode="{$arrayMatchs[i].Periode|default:''}"
															title="{#Match_en_attente#}">{$arrayMatchs[i].Statut}</span>
														<span class="scoreProvisoire hidden"
															title="{#scoreProvisoire#}">{$arrayMatchs[i].ScoreDetailA} -
															{$arrayMatchs[i].ScoreDetailB}</span>
													{/if}
												</td>
												<td>
													<span class='directInput score'
														tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}6"
														Id="ScoreB-{$arrayMatchs[i].Id}-text">{$arrayMatchs[i].ScoreB}</span>
												</td>
												<td>
													<span class="directInput equipe{if $arrayMatchs[i].Id_equipeB < 1} undefTeam{/if}"
														tabindex="1{$smarty.section.i.iteration|string_format:'%02d'}9"
														Id="EquipeB-{$arrayMatchs[i].Id}-text" data-match="{$arrayMatchs[i].Id}"
														data-journee="{$arrayMatchs[i].Id_journee}"
														data-idequipe="{$arrayMatchs[i].Id_equipeB}"
														data-equipe="B">{$arrayMatchs[i].EquipeB}</span>
													<br />
													<a href="GestionMatchEquipeJoueur.php?idMatch={$arrayMatchs[i].Id}&codeEquipe=B"
														title="{#Composition_equipe#} B"><img height="20"
															src="../img/b_compo_match.png"></a>
												</td>
												<td>
													<span
														class="directInput arbitre{if $arrayMatchs[i].Arbitre_principal != '-1' && $arrayMatchs[i].Matric_arbitre_principal == 0} pbArb{/if}"
														tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}6"
												...
												<td>
													<span
														class="directInput arbitre{if $arrayMatchs[i].Arbitre_secondaire != '-1' && $arrayMatchs[i].Matric_arbitre_secondaire == 0} pbArb{/if}"
														tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}6"
												...
												<td>
													<img src="../img/imprime{$arrayMatchs[i].Imprime}.png" class="imprimMatch" height="20"
														data-valeur="{$arrayMatchs[i].Imprime}" data-id="{$arrayMatchs[i].Id}"
														title="{if $arrayMatchs[i].Imprime == 'O'}{#Imprime#}{/if}">
													{if $arrayMatchs[i].CoeffA != 1 || $arrayMatchs[i].CoeffB != 1}<br>{$arrayMatchs[i].CoeffA}/{$arrayMatchs[i].CoeffB}{/if}
												</td>
												<td>
													<a href="#" class="showOn"
														onclick="RemoveCheckbox('formJournee', '{$arrayMatchs[i].Id}');return false;"><img
															height="20" src="../img/glyphicons-17-bin.png" title="{#Supprimer#}">
													</a>
												</td>
											{else}
												<td>
													<span class='directInputOff numMatch' Id="Numero_ordre-{$arrayMatchs[i].Id}-text"
														tabindex='1{$smarty.section.i.iteration|string_format:"%02d"}0'>{$arrayMatchs[i].Numero_ordre}</span>
												</td>
												<td width=80>
													<a href="#" class="showOff" onclick="ParamMatch({$arrayMatchs[i].Id});">
														<img height="20" src="../img/glyphicons-31-pencil.png" title="{#Editer#}">
													</a>
													<a href="FeuilleMatchMulti.php?listMatch={$arrayMatchs[i].Id}" Target="_blank">
														<img height="20" src="../img/pdf.png" title="{#Feuille_marque#} (Pdf)">
													...
													{if $profile <= 1}
														<a href="#"
															onclick="window.open('FeuilleMarque3.php?idMatch={$arrayMatchs[i].Id}','FeuilleV3'); return false;">
															<img height="20" src="../img/ipad-3.png" title="{#Feuille_marque_en_ligne#} (V3)">
														</a>
													{/if}
												</td>
												<td>
													<span class='directInputOff date{if $lang=='en'}EN{/if}'
														Id="Date_match-{$arrayMatchs[i].Id}-date{if $lang=='en'}EN{/if}"
														tabindex="1{$smarty.section.i.iteration|string_format:'%02d'}1">{$arrayMatchs[i].Date_match}</span>
													<br>
												...
												<td title="{$arrayMatchs[i].Code_competition}">
													<span class="compet">
														{if $arrayMatchs[i].Soustitre2 != ''}{$arrayMatchs[i].Soustitre2}
														{else}{$arrayMatchs[i].Code_competition}
														{/if}</span>
												</td>
												{if $PhaseLibelle == 1}
													<td>
														<span class="directInputOff phase"
															tabindex='1{$smarty.section.i.iteration|string_format:"%02d"}6'
															data-match="{$arrayMatchs[i].Id}" data-idphase="{$arrayMatchs[i].Id_journee}"
															data-phase="{$arrayMatchs[i].Phase}">{$arrayMatchs[i].Phase|default:'&nbsp;'}</span>
													</td>
													<td>
														<span class='directInputOff text eq'
															tabindex='1{$smarty.section.i.iteration|string_format:"%02d"}3'
															Id="Libelle-{$arrayMatchs[i].Id}-text">{$arrayMatchs[i].Libelle|default:'&nbsp;'}</span>
													</td>
												{else}
													...
													<td>
														<span class="lieu">{$arrayMatchs[i].Lieu|default:'&nbsp;'}</span></td>
												{/if}
												<td>
													<img class="typeMatchOff" data-valeur="{$arrayMatchs[i].Type}"
														data-id="{$arrayMatchs[i].Id}" src="../img/type{$arrayMatchs[i].Type}.png"
														title="{if $arrayMatchs[i].Type == 'C'}{#Match_de_classement#}{else}{#Match_eliminatoire#}{/if}"
														height="23">
												</td>
												<td>
													<span class='directInputOff terrain'
														tabindex="1{$smarty.section.i.iteration|string_format:'%02d'}4"
														Id="Terrain-{$arrayMatchs[i].Id}-text">{$arrayMatchs[i].Terrain|default:'&nbsp;'}</span>
												</td>
												<td>
													<span class="directInputOff equipe {if $arrayMatchs[i].Id_equipeA < 1}undefTeam{/if}"
														tabindex="1{$smarty.section.i.iteration|string_format:'%02d'}9"
														Id="EquipeA-{$arrayMatchs[i].Id}-text" data-match="{$arrayMatchs[i].Id}"
														data-journee="{$arrayMatchs[i].Id_journee}"
												...
												<td>
													<span class='directInputOff score'
														tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}5"
														Id="ScoreA-{$arrayMatchs[i].Id}-text">{$arrayMatchs[i].ScoreA}</span>
												</td>
												<td class='color{$arrayMatchs[i].Validation}2'>
													{if $profile <= 6 && $AuthModif == 'O'}
														<img class="verrouMatch" data-valeur="{$arrayMatchs[i].Validation}"
													...
													{/if}
													{if $arrayMatchs[i].Statut == 'ON'}
														<span class="directStatutMatchOff statutMatchOn" data-statut="ON"
															data-id="{$arrayMatchs[i].Id}" data-periode="{$arrayMatchs[i].Periode|default:''}"
														...
														<span class="scoreProvisoire"
															...
															{$arrayMatchs[i].ScoreDetailB}</span>
													{elseif $arrayMatchs[i].Statut == 'END'}
														<span class="directStatutMatchOff statutMatchOn" data-statut="END"
															data-id="{$arrayMatchs[i].Id}" data-periode="{$arrayMatchs[i].Periode|default:''}"
															title="{#Match_termine#}">{$arrayMatchs[i].Statut}</span>
														<span class="scoreProvisoire"
															title="{#scoreProvisoire#}">{$arrayMatchs[i].ScoreDetailA} -
															{$arrayMatchs[i].ScoreDetailB}</span>
													{else}
														<span class="directStatutMatchOff scoreProvisoire" data-statut="ATT"
															data-id="{$arrayMatchs[i].Id}" data-periode="{$arrayMatchs[i].Periode|default:''}"
															title="{#Match_en_attente#}">{$arrayMatchs[i].Statut}</span>
														<span class="scoreProvisoire hidden"
															title="{#scoreProvisoire#}">{$arrayMatchs[i].ScoreDetailA} -
												...
												<td>
													<span class="directInputOff equipe{if $arrayMatchs[i].Id_equipeB < 1} undefTeam{/if}"
														tabindex="1{$smarty.section.i.iteration|string_format:'%02d'}9"
														Id="EquipeB-{$arrayMatchs[i].Id}-text" data-match="{$arrayMatchs[i].Id}"
														data-journee="{$arrayMatchs[i].Id_journee}"
														data-idequipe="{$arrayMatchs[i].Id_equipeB}"
													...
													<br />
													<a href="GestionMatchEquipeJoueur.php?idMatch={$arrayMatchs[i].Id}&codeEquipe=B"
														title="{#Composition_equipe#} B">
														<img height="20" src="../img/b_compo_match.png">
													</a>
												</td>
												<td>
													<span
														class="directInputOff arbitre{if $arrayMatchs[i].Arbitre_principal != '-1' && $arrayMatchs[i].Matric_arbitre_principal == 0} pbArb{/if}"
														tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}6"
														data-id="Arbitre_principal" data-match="{$arrayMatchs[i].Id}"
														data-journee="{$arrayMatchs[i].Id_journee}"
														Id="Arbitre_principal-{$arrayMatchs[i].Id}-text">{$arrayMatchs[i].Arbitre_principal|replace:' (':' <br />('|replace:') ':')<br /> '|replace:'-1':''}</span>
												...
												<td>
													<span
														class="directInputOff arbitre{if $arrayMatchs[i].Arbitre_secondaire != '-1' && $arrayMatchs[i].Matric_arbitre_secondaire == 0} pbArb>{/if}"
														tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}6"
														data-id="Arbitre_secondaire" data-match="{$arrayMatchs[i].Id}"
														data-journee="{$arrayMatchs[i].Id_journee}"
														Id="Arbitre_secondaire-{$arrayMatchs[i].Id}-text">{$arrayMatchs[i].Arbitre_secondaire|replace:' (':' <br />('|replace:') ':')<br /> '|replace:'-1':''}</span>
												...
												<td>
													<img src="../img/imprime{$arrayMatchs[i].Imprime}.png" class="imprimMatchOff"
														height="20" data-valeur="{$arrayMatchs[i].Imprime}" data-id="{$arrayMatchs[i].Id}"
														title="{if $arrayMatchs[i].Imprime == 'O'}{#Imprime#}{/if}">
													{if $arrayMatchs[i].CoeffA != 1 || $arrayMatchs[i].CoeffB != 1}<br>{$arrayMatchs[i].CoeffA}/{$arrayMatchs[i].CoeffB}{/if}
												</td>
												<td>
													<a href="#" class="showOff" onclick="RemoveCheckbox('formJournee', '{$arrayMatchs[i].Id}');return false;">
														<img height="20" src="../img/glyphicons-17-bin.png" title="{#Supprimer#}">
													</a>
												</td>
											{/if}
										{elseif $arrayMatchs[i].MatchAutorisation == 'O' && $profile == 9 && $AuthModif == 'O'}
											<td>
												<img height="24" src="../img/oeil2{$arrayMatchs[i].Publication|default:'N'}.gif"
													title="{if $arrayMatchs[i].Publication == 'O'}{#Public#}{else}{#Prive#}{/if}">
											</td>
											{if $arrayMatchs[i].Validation != 'O'}
												<td>{$arrayMatchs[i].Numero_ordre}</td>
												<td width=80>
													<a href="FeuilleMatchMulti.php?listMatch={$arrayMatchs[i].Id}" Target="_blank"><img
															height="20" src="../img/pdf.png" title="{#Feuille_marque#} (Pdf)"></a>
													<br />
													<a href="hey#"
														onclick="window.open('FeuilleMarque2.php?idMatch={$arrayMatchs[i].Id}','FeuilleV2'); return false;"><img
															height="20" src="../img/glyphicons-163-ipad.png"
															title="{#Feuille_marque_en_ligne#}">
													</a>
													{if $profile <= 1}
														<a href="#"
												...
												<td title="{$arrayMatchs[i].Code_competition}"><span class="compet">
														{if $arrayMatchs[i].Soustitre2 != ''}{$arrayMatchs[i].Soustitre2}
														{else}{$arrayMatchs[i].Code_competition}
														{/if}</span>
												</td>
												...
												{/if}
												<td>
													<img src="../img/type{$arrayMatchs[i].Type}.png"
														title="{if $arrayMatchs[i].Type == 'C'}{#Match_de_classement#}{else}{#Match_eliminatoire#}{/if}">
												</td>
												...
												<td>
													...
													<br />
													<a href="GestionMatchEquipeJoueur.php?idMatch={$arrayMatchs[i].Id}&codeEquipe=A"
														title="{#Composition_equipe#} A"><img height="20"
															src="../img/b_compo_match.png"></a>
												</td>
												<td class='directInput score'
													tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}5"><span
														Id="ScoreA-{$arrayMatchs[i].Id}-text">{$arrayMatchs[i].ScoreA}</span></td>
												<td class='color{$arrayMatchs[i].Validation}2'>
													<img height="25" src="../img/verrou2{$arrayMatchs[i].Validation|default:'N'}.gif"
														title="{#Verrouiller#}">
													{if $arrayMatchs[i].Statut == 'ON'}
												...
												<td class='directInput score'
													tabindex="2{$smarty.section.i.iteration|string_format:'%02d'}6"><span
												...
												<td>
													{if $arrayMatchs[i].Arbitre_principal != '-1'}
														{$arrayMatchs[i].Arbitre_principal|replace:'(':'<br>('}
													{else}
														&nbsp;
													{/if}
												</td>
												<td>
													{if $arrayMatchs[i].Arbitre_secondaire != '-1'}
														{$arrayMatchs[i].Arbitre_secondaire|replace:'(':'<br>('}
													{else}
														&nbsp;
													{/if}
												</td>
												<td>
													<img src="../img/imprime{$arrayMatchs[i].Imprime}.png" height="20"
														title="{if $arrayMatchs[i].Imprime == 'O'}{#Imprime#}{/if}">
													{if $arrayMatchs[i].CoeffA != 1 || $arrayMatchs[i].CoeffB != 1}<br>{$arrayMatchs[i].CoeffA}/{$arrayMatchs[i].CoeffB}{/if}
												</td>
												<td>&nbsp;</td>
										...
										{else}
											<td>
												<img height="25" src="../img/oeil2{$arrayMatchs[i].Publication|default:'N'}.gif"
													{if $arrayMatchs[i].Publication == 'O'}{#Public#} {else}{#Prive#} {/if}>
											</td>
											<td>{$arrayMatchs[i].Numero_ordre}</td>
											<td width=80>
												<a href="FeuilleMatchMulti.php?listMatch={$arrayMatchs[i].Id}" Target="_blank"><img
											...
											<td>
												{if $arrayMatchs[i].Arbitre_principal != '-1'}{$arrayMatchs[i].Arbitre_principal|replace:'(':'<br>('}{else}&nbsp;{/if}
											</td>
											<td>
