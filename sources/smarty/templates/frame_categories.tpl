                    ...
                    <table class='tableau table table-striped table-condensed table-responsive table-hover display compact'>
                        <thead>
                            <tr class="text-center">
                                <th colspan="9" class="text-center bg-primary text-white" width="100%">{#Categorie#} {$arrayCompetition[0].Soustitre2}</th>
                            </tr>
                            <tr>
                        ...
                        <tbody>
                            {section name=j loop=$arrayMatchs}
                                {assign var='Match' value=$arrayMatchs[j]}
...
*}
                                <tr class='{$Match.past}'>
                                    {if $Match.Numero_ordre}
                                        <td class="text-center" data-order="{$Match.Heure_match}" data-filter="{$Match.Heure_match}">
                                            <span class="badge">{$Match.Heure_match}</span>
                                        </td>
                                        <td>{$Match.Numero_ordre}</td>
                                        <td class="terrain text-center" data-cat="{$Match.Terrain}">{$Match.Terrain}</td>
                                        <td>{$Match.Phase|default:'&nbsp;'}</td>
                                        <td class="text-center" data-filter="{$Match.EquipeA|default:'&nbsp;'}">
                                            <a class="btn btn-xs btn-default equipe">
                                                {$Match.EquipeA|default:'&nbsp;'}
                                            </a>
                                        </td>
                                        <td class="text-center"><span title="{$Match.Id}">
                                            {if $Match.Validation == 'O' && $Match.ScoreA != '?' && $Match.ScoreA != '' && $Match.ScoreB != '?' && $Match.ScoreB != ''}
                                                <button type="button" class="btn btn-success btn-xs" title="{#END#}">
                                                       {$Match.ScoreA|replace:'?':'&nbsp;'|default:'&nbsp;'} - {$Match.ScoreB|replace:'?':'&nbsp;'|default:'&nbsp;'}
                                                </button>
                                            {elseif $Match.Statut == 'ON' && $Match.Validation != 'O'}
                                                <button type="button" class="btn btn-warning btn-xs scoreProvisoire" title="{#scoreProvisoire#}">
                                                       {$Match.ScoreA|replace:'?':'&nbsp;'|default:'&nbsp;'} - {$Match.ScoreB|replace:'?':'&nbsp;'|default:'&nbsp;'}
                                                </button>
                                            {elseif $Match.Statut == 'END' && $Match.Validation != 'O'}
                                                <button type="button" class="btn btn-warning btn-xs scoreProvisoire" title="{#scoreProvisoire#}">
                                                       {$Match.ScoreA|replace:'?':'&nbsp;'|default:'&nbsp;'} - {$Match.ScoreB|replace:'?':'&nbsp;'|default:'&nbsp;'}
                                                </button>
                                            {else}
                                        ...
                                        <td class="text-center" data-filter="{$Match.EquipeB|default:'&nbsp;'}">
                                            <a class="btn btn-xs btn-default equipe">
                                                {$Match.EquipeB|default:'&nbsp;'}
                                            </a>
                                        </td>
                                        <td class="text-center">
                                            <a class="btn btn-xs btn-default arbitre">{$Match.Arbitre_principal|default:'&nbsp;'}</a>
                                        </td>
                                        <td class="text-center">
                                            <a class="btn btn-xs btn-default arbitre">{$Match.Arbitre_secondaire|default:'&nbsp;'}</a>
                                        </td>
                                    {else}
