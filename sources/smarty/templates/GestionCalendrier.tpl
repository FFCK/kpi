	<div class="main">
		<form method="POST" action="GestionCalendrier.php" name="formCalendrier" id="formCalendrier"
						...
						<td align="right" rowspan=2>
							{if $profile <= 3 && $AuthModif == 'O'}
								{if $modeEvenement eq '1'}
									{#Mode_normal#}<input type="radio" onclick="changeModeEvenement();" name="choixModeEvenement"
										name="choixModeEvenement" value="1" Checked>
									<br>{#Association_evts#}<input type="radio" onclick="changeModeEvenement();" name="choixModeEvenement"
										value="2">
								{else}
						...
						<td>
							...
							<select name="competitionOrder" onChange="changeCompetitionOrder();">
								{section name=i loop=$arrayCompetitionOrder}
									<Option Value="{$arrayCompetitionOrder[i].Code}" {$arrayCompetitionOrder[i].Selection}>
										{$arrayCompetitionOrder[i].Libelle}</Option>
								{/section}
							</select>
						</td>
						<td>
						...
						{if $profile <= 4 && $AuthModif == 'O'}
							<td>
								<a href="#" onclick="ParamJournee(0);">
									<img align="bottom" height="20" src="../img/glyphicons-191-plus-sign.png"
										title="{#Ajouter_une_journee#}">
									{#Ajouter_une_journee#}
								</a>
							</td>
						{/if}
				...
				<div class='blocMiddle'>
					<table width="100%">
						<tr>
							<td>
								{#Selection#} :&nbsp;
								<a href="#" onclick="setCheckboxes('formCalendrier', 'checkJournee', true);return false;"><img height="22"
										src="../img/glyphicons-155-more-checked.png" alt="{#Tous#}" title="{#Tous#}" border="0"></a>
								&nbsp;
								<a href="#" onclick="setCheckboxes('formCalendrier', 'checkJournee', false);return false;"><img height="22"
										src="../img/glyphicons-155-more-windows.png" alt="{#Aucun#}" title="{#Aucun#}" border="0"></a>
								&nbsp;
								<a href="#" onclick="SelectedCheckboxes('formCalendrier', 'checkJournee');publiMultiJournees();"
									title="{#Publier#}"><img height="25" src="../img/oeil2.gif" alt="{#Publier#}" border="0"></a>
								&nbsp;
								<a href="#" onclick="RemoveCheckboxes('formCalendrier', 'checkJournee')" title="{#Supprimer#}"><img
										height="25" src="../img/glyphicons-17-bin.png" alt="{#Supprimer#}" border="0"></a>
							</td>
							<td>
						...
						<thead>
							<tr>
								{if $profile <= 3 && $AuthModif == 'O'}
									<th>&nbsp;</th>
								{/if}
								<th width=18><img height="18" src="../img/oeil2.gif" alt="{#Publier#} ?" title="{#Publier#} ?"
										border="0"></th>
								<th>Id</th>
								<th>&nbsp;</th>
								<th>Compet./Phase</th>
								{if $competition.Code_typeclt == "CP"}
									<th>{#Niv#} <img width="12" src="../img/b_help.png" {if $lang == "en"} title="Level : Importance of the phase in the géléral classification. For example :
                                            ...
                                            <br>even if they are played before." {else} title="Niveau : Importance de la phase dans le classement géléral. Par exemple :
                                            ...
                                            <br>même si elles se jouent avant." {/if}></th>
									<th>{#Tour#} <img width="12" src="../img/b_help.png" {if $lang == "en"} title="Round : stage in the game chart. For example:
											...
											{else} title="Tour : étape dans le schéma du système de jeu. Par exemple :
											...
											{/if}></th>
									<th>{#Equipes#} <img width="12" src="../img/b_help.png" {if $lang == "en"}
											title="Team count (for classifying groups)" {else}
											title="Nombre d'équipes (pour les poules de classement)" {/if}></th>
								{/if}
								<th>Type</th>
								...
								<tr class='{cycle values="impair,pair"}'>
									{if $modeEvenement eq '1'}
										{if $arrayJournees[i].Autorisation eq true && $profile <= 4 && $AuthModif == 'O'}
											{if $profile <= 3 && $AuthModif == 'O'}
												<td><input type="checkbox" name="checkJournee" value="{$arrayJournees[i].Id}"
														id="checkDelete{$smarty.section.i.iteration}" /></td>
											{/if}
											<td class="color{$arrayJournees[i].Publication|default:'N'}2">
												<img class="publiJournee" data-valeur="{$arrayJournees[i].Publication}"
													data-id="{$arrayJournees[i].Id}" height="25"
													src="../img/oeil2{$arrayJournees[i].Publication|default:'N'}.gif"
													title="{if $arrayJournees[i].Publication == 'O'}{#Public#}{else}{#Prive#}{/if}" />
											</td>
											...
											{if $competition.Code_typeclt == "CP"}
												...
												<td>
													<span class='directInput' data-type="tel" data-target="Etape" data-id="{$arrayJournees[i].Id}"
														data-value="{$arrayJournees[i].Etape}">{$arrayJournees[i].Etape}</span>
												</td>
												<td>
													<span class='directInput' data-type="tel" data-target="Nbequipes" data-id="{$arrayJournees[i].Id}"
														data-value="{$arrayJournees[i].Nbequipes}">{$arrayJournees[i].Nbequipes}</span>
												</td>
											{/if}
											<td><img class="typeJournee" data-valeur="{$arrayJournees[i].Type}" data-id="{$arrayJournees[i].Id}"
													src="../img/type{$arrayJournees[i].Type}.png"
											...
											<td>
												<span class='directInput' data-type="date{if $lang=='en'}EN{/if}" data-target="Date_debut"
													data-id="{$arrayJournees[i].Id}"
													data-value="{$arrayJournees[i].Date_debut}">{$arrayJournees[i].Date_debut}</span>
												-
												<span class='directInput' data-type="date{if $lang=='en'}EN{/if}" data-target="Date_fin"
													data-id="{$arrayJournees[i].Id}"
													data-value="{$arrayJournees[i].Date_fin}">{$arrayJournees[i].Date_fin}</span>
											...
											<td><a href="#" onclick="RemoveCheckbox('formCalendrier', '{$arrayJournees[i].Id}');return false;"><img
														height="20" src="../img/glyphicons-17-bin.png" alt="{#Supprimer#}" title="{#Supprimer#}"></a></td>
										{else}
											{if $profile <= 3 && $AuthModif == 'O'}
												<td>&nbsp;</td>
											{/if}
											<td class="color{$arrayJournees[i].Publication|default:'N'}2">
												<img height="25" src="../img/oeil2{$arrayJournees[i].Publication|default:'N'}.gif"
													title="{if $arrayJournees[i].Publication == 'O'}{#Public#}{else}{#Prive#}{/if}">
											</td>
											<td>{$arrayJournees[i].Id}</td>
											<td style="text-align:left">
											...
											{/if}
											<td><img src="../img/type{$arrayJournees[i].Type}.png"
													title="{if $arrayJournees[i].Type == 'C'}{#Classement#}{else}{#Elimination#}{/if}" height="23" />
											</td>
											...
											<td><a href="GestionInstances.php?idJournee={$arrayJournees[i].Id}" title="{#Officiels#}"><img
														height="18" src="../img/glyphicons-social-63-buffer.png" alt="{#Officiels#}"></a>
											<td style="text-align:left">
												{if $arrayJournees[i].Responsable_insc != ''}{#RC#}: {$arrayJournees[i].Responsable_insc}<br />{/if}
												{if $arrayJournees[i].Responsable_R1 != ''}{#R1#}: {$arrayJournees[i].Responsable_R1}<br />{/if}
												{if $arrayJournees[i].Delegue != ''}{#Delegue#}: {$arrayJournees[i].Delegue}<br />{/if}
									...
									{elseif $arrayJournees[i].Autorisation eq true && $profile <= 4 && $AuthModif == 'O'}
										...
										<td>{$arrayJournees[i].Id}</td>
										<td class="rouge">
											<input type="checkbox" class="checkassoc2" data-id="{$arrayJournees[i].Id}"
												{$arrayJournees[i].Checked} />
										</td>
										<td>{$arrayJournees[i].Code_competition}
											-
											{$arrayJournees[i].Phase}
									...
									{else}
										...
										<td><img src="../img/type{$arrayJournees[i].Type}.png"
												title="{if $arrayJournees[i].Type == 'C'}Classement{else}Elimination{/if}" /></td>
										<td>{$arrayJournees[i].Code_competition}
											-
											{$arrayJournees[i].Phase}
										</td>
