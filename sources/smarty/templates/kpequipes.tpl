{if $codeCompet != '' || $Equipe == 0}
    <div class="container-fluid" id="navGroup">
        <nav class="col-md-12 navbar navbar-custom">
            <div class='pull-left'>
                <a class="btn btn-primary btn-navigation" href='javascript:history.back()'>< {#Retour#}</a>
            </div>
        </nav>
        ...
        <article class="col-md-6 padTopBottom" id="equipePalmares">        
            ...
            {section  name=i loop=$arraySaisons}
                ...
                <table class='table table-striped table-hover table-condensed' id='tableMatchs'>
                    <caption><h4>{$Saison}</h4></caption>
                    <tbody>        
                        {section  name=j loop=$arrayPalmares[$Saison]}
                            {if $arrayPalmares[$Saison][j].Code_tour == 10}
                                <tr>
                                    <td>
                                        <a class="btn btn-xs btn-default" {if !$arrayCompo}href='kpclassements.php?Compet={$arrayPalmares[$Saison][j].Code}&Group={$arrayPalmares[$Saison][j].Code_ref}&Saison={$arrayPalmares[$Saison][j].Saison}' title='{#Classement#}'{/if}>
                                            {$arrayPalmares[$Saison][j].Competitions}
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        {if $arrayPalmares[$Saison][j].Classt > 0 && $arrayPalmares[$Saison][j].Classt <= 3}
                                            <img width="20" src="img/medal{$arrayPalmares[$Saison][j].Classt}.gif" alt="{$arrayPalmares[$Saison][j].Classt}" title="{$arrayPalmares[$Saison][j].Classt}" />
                                        {else}
