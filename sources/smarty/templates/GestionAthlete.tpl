		<iframe name="iframeRechercheLicenceIndi2" id="iframeRechercheLicenceIndi2" SRC="RechercheLicenceIndi2.php"
			scrolling="auto" width="950" height="450" FRAMEBORDER="yes"></iframe>
		<div class="main">
			<form method="POST" action="GestionAthlete.php" name="formAthlete" id="formAthlete" enctype="multipart/form-data">
				...
				<div class='blocTop'>
					...
					{if $profile <= 6}
						...
						<a href="#" id="rechercheAthlete"><img height="18" src="../img/glyphicons-28-search.png"
					...
					<div class='blocMiddle'>
						...
						{if $profile <= 2 && $Courreur.Matric > 2000000}
							<div class='blocTop'>
								...
								{#Date_naissance#}:<input type="text" name="update_naissance" id="update_naissance" size="10" maxlength="10"
						...
						<p><b>{#Saison#}:</b>
							<select name="SaisonAthlete" id="SaisonAthlete" onChange="submit()">
								{section name=i loop=$arraySaison}
									<Option Value="{$arraySaison[i].Code}" {if $arraySaison[i].Code eq $SaisonAthlete}selected{/if}>
											...
											<tbody>
												{section name=i loop=$Titulaire}
													<tr>
														...
														{if $lang == 'en'}
															<td>#{$Titulaire[i].Num}
																{$Titulaire[i].Capitaine|replace:'C':'Cap'|replace:'E':'Coach'|replace:'A':'Ref.'|replace:'X':'Unavailable'|replace:'-':''}
															</td>
														{else}
															<td>n°{$Titulaire[i].Num}
																{$Titulaire[i].Capitaine|replace:'C':'Cap'|replace:'E':'Entraineur'|replace:'A':'Arbitre'|replace:'X':'INACTIF'|replace:'-':''}
															</td>
														{/if}
												...
												{section name=i loop=$Arbitrages}
													<tr>
														...
														<td>{$Arbitrages[i].Numero_ordre}
															{if $profile <= 3}
																<a href="FeuilleMatchMulti.php?listMatch={$Arbitrages[i].Identifiant}" target="_blank"><img
																		width="10" src="../img/b_plus.png" alt="{#Feuille_marque#}" title="{#Feuille_marque#}" /></a>
															{/if}
														</td>
												...
												{section name=i loop=$OTM}
													<tr>
														...
														<td>{$OTM[i].Numero_ordre}
															{if $profile <= 3}
																<a href="FeuilleMatchMulti.php?listMatch={$OTM[i].Identifiant}" target="_blank"><img width="10"
																		src="../img/b_plus.png" alt="{#Feuille_marque#}" title="{#Feuille_marque#}" /></a>
															{/if}
														</td>
														{if $OTM[i].ScoreOK == 'O'}
															<td>{$OTM[i].Sec}</td>
										...
										<table class='tableau2'>
											...
											<tbody>
												{section name=i loop=$Joueur}
													{if $Joueur[i].ScoreOK == 'O'}
														<tr>
															...
															<td>
																...
																{if $profile <= 3}
																	<a href="FeuilleMatchMulti.php?listMatch={$Joueur[i].Identifiant}" target="_blank"><img width="10"
																			src="../img/b_plus.png" alt="{#Feuille_marque#}" title="{#Feuille_marque#}" /></a>
																{/if}
															</td>
															...
															{if $lang == 'en'}
																<td><i>#{$Joueur[i].Num}
																		{$Joueur[i].Capitaine|replace:'C':'Cap'|replace:'E':'Coach'|replace:'A':'Ref.'|replace:'-':''}</i>
																</td>
															{else}
																<td><i>n°{$Joueur[i].Num}
																		{$Joueur[i].Capitaine|replace:'E':'Entraineur'|replace:'A':'Arbitre'|replace:'C':'Cap'|replace:'-':''}</i>
																</td>
															{/if}
															...
															{if $Joueur[i].Rouge_definitif > 0}<td class='rouge'>{$Joueur[i].Rouge_definitif}</td>
															{else}<td>
															</td>{/if}
													...
													{else}
														<tr class="gris">
															<td><i>{$Joueur[i].Date_match}</i></td>
															<td><i>{$Joueur[i].Code_competition}</i></td>
															<td><i>
																	{$Joueur[i].Numero_ordre}
																	{if $profile <= 3}
																		<a href="FeuilleMatchMulti.php?listMatch={$Joueur[i].Identifiant}" target="_blank"><img
																				width="10" src="../img/b_plus.png" alt="{#Feuille_marque#}"
																				title="{#Feuille_marque#}" /></a>
																	{/if}</i>
															</td>
