									...
									*}									
									<td class='color{$arrayEvenement[i].Publication}2'>
										<a href="#" Id="Publication{$arrayEvenement[i].Id}" onclick="publiEvt({$arrayEvenement[i].Id},'{$arrayEvenement[i].Publication}')">
											<img width="24" src="../img/oeil2{$arrayEvenement[i].Publication}.gif" alt="{#Publier#}" title="{#Publier#}" />
										</a>
									</td>
									...
									{if $profile <= 1}
										<td><a href="#" onclick="RemoveCheckbox('formEvenement', '{$arrayEvenement[i].Id}');return false;"><img height="20" src="../img/glyphicons-17-bin.png" alt="{#Supprimer#}" title="{#Supprimer#}" /></a></td>
									{else}
										<td>&nbsp;</td>
									{/if}
								</tr>
				...
				<table width=100%>
					...
					<tr>
						<td>
							...
							<input type="text" class='date' name="Date_debut" value="{$Date_debut}" id="Date_debut" onfocus="displayCalendar(document.forms[0].Date_debut,{if $lang=='en'}'yyyy-mm-dd'{else}'dd/mm/yyyy'{/if},this)" />
						</td>
						<td>
							<label for="Date_fin">{#Date_fin#} :</label>
							<input type="text" class='date' name="Date_fin" value="{$Date_fin}" id="Date_fin" onfocus="displayCalendar(document.forms[0].Date_fin,{if $lang=='en'}'yyyy-mm-dd'{else}'dd/mm/yyyy'{/if},this)" />
						</td>
					</tr>
					<tr>
						{if $idEvenement != -1}
