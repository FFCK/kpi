				...
				<div class='blocRight Right3'>
					<table class='tableau2'>
							...
							<td><label for="saisonOrigine">Saison Origine</label>
								<select name="saisonOrigine" onchange="submit()">
									{section name=i loop=$arraySaisons}
										<Option Value="{$arraySaisons[i].Code}" {if $arraySaisons[i].Code == $saisonOrigine}selected{/if}>{$arraySaisons[i].Code}</Option>
									{/section}
								</select>
							</td>
						...
						<tr>
							<td colspan=2 align="center">
								...
								{else}
									{section name=i loop=$arrayJournees}
										{if !$smarty.section.i.first}{if $arrayJournees[i].Niveau != $niveauTmp}<br>{else} | {/if}{/if}
						...
						<tr>
							<td><label for="saisonDestination">Saison Destination</label>
								<select name="saisonDestination" onchange="submit()">
									{section name=i loop=$arraySaisons}
										<Option Value="{$arraySaisons[i].Code}" {if $arraySaisons[i].Code == $saisonDestination}selected{/if}>{$arraySaisons[i].Code}</Option>
									{/section}
								</select>
							</td>
					...
					<table class='tableau2'>
						...
						<tr>
							...
							<td class="vert"><label for="Departement">Département</label><input type="text" class="dpt" name="Departement" value="{$Departement}"/></td>
						</tr>
						<tr>
...
-->						<tr>
							<td><br><label for="Organisateur">Club Organisateur</label><input type="text" name="Organisateur" value="{$Organisateur}"/></td>
							<td><br><label for="Plan_eau">Plan d'eau</label><input type="text" name="Plan_eau" value="{$Plan_eau}"/></td>
						</tr>
						<tr>
							<td><label for="Responsable_R1">Responsable local R1</label><input type="text" name="Responsable_R1" value="{$Responsable_R1}"/></td>
							<td><label for="Responsable_insc">Responsable insc. RZ</label><input type="text" name="Responsable_insc" value="{$Responsable_insc}"/></td>
						</tr>
						<tr>
