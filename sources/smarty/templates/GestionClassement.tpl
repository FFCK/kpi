		<div class="main">
			<form method="POST" action="GestionClassement.php" name="formClassement" id="formClassement"
					...
					<select name="saisonTravail" id="saisonTravail" onChange="sessionSaison()">
						{section name=i loop=$arraySaison}
							<Option Value="{$arraySaison[i].Code}" {if $arraySaison[i].Code eq $sessionSaison}selected{/if}>
								{$arraySaison[i].Code}{if $arraySaison[i].Code eq $sessionSaison} ({#actuelle#}){/if}</Option>
						{/section}
					</select>
								...
								{section name=i loop=$arrayEquipe}
									<tr height="17" class='{cycle values="impair,pair"}'>
										{if $smarty.section.i.iteration <= $Qualifies}
											<td class='vert'><img width="16" src="../img/up.gif" alt="Qualifi&eacute;" title="Qualifi&eacute;" />
											</td>
										{elseif $smarty.section.i.iteration > $smarty.section.i.total - $Elimines}
											<td class='rouge'><img width="16" src="../img/down.gif" alt="Elimin&eacute;s" title="Elimin&eacute;s" />
											</td>
										{else}
											<td>&nbsp;</td>
										{/if}
										<td>
											{if $profile <= 4 && $AuthModif == 'O'}
												<input type="checkbox" name="checkClassement" value="{$arrayEquipe[i].Id}"
											...
											<tr height="17" class='{cycle values="impair,pair"}'>
												<td>
													{if $arrayEquipe_journee[i].J == '0' && $profile <= 4 && $AuthModif == 'O'}
														<a href="" class="dropTeam" data-Id="{$arrayEquipe_journee[i].Id}"
															data-Id_journee="{$arrayEquipe_journee[i].Id_journee}"><img height="20"
																src="../img/glyphicons-17-bin.png" title="{#Supprimer#}" border="0"></a>
													{else}
														&nbsp;
													{/if}
												</td>
							...
							<tbody>
								{section name=i loop=$arrayEquipe_publi}
									<tr height="17" class='{cycle values="impair2,pair2"}'>
										...
										<td>
											{if $profile <= 4 && $AuthModif == 'O'}
												<input type="checkbox" name="checkClassement" value="{$arrayEquipe_publi[i].Id}"
													id="checkClassement{$smarty.section.i.iteration}" />
											{/if}
										</td>
										{if (isset($Code_niveau) && $Code_niveau) == 'INT'}
				...
				<div class='blocRight'>
					<table width="100%">
						...
						<tr>
							<td align='center' colspan=4>
								{if $Date_calcul == '00/00/00 &agrave; 00h00'}{#Classement_non_calcule#}
								{else}
							...
							<tr>
								<td colspan=4 align='center'>
									<input class="bigbutton" type="button" onclick="publicationClt();" name="Publier"
										value="{#Publier_nouveau_classement#}">
								</td>
							</tr>
						{/if}
					...
					<table width="100%">
						{if $typeCompetition=='Championnat'}
							...
							<tr>
								<td align='left'>
									<a href="FeuilleCltChpt.php" Target="_blank"><img height="30" src="../img/pdf.png"
											title="{#Classement_general#} - admin" /></a>
								</td>
								<td colspan=2 align='center'>{#Classement_general#}</td>
								<td align='right'>
									{if $Code_uti_publication != ''}
										<a href="../PdfCltChpt.php" Target="_blank"><img height="30" src="../img/pdf.png"
							...
							<tr>
								<td align='left'>
									<a href="FeuilleCltChptDetail.php" Target="_blank"><img height="30" src="../img/pdf.png"
											title="{#Detail_par_equipe#} - admin" /></a>
								</td>
								<td colspan=2 align='center'>{#Detail_par_equipe#}</td>
								<td align='right'>
									{if $Code_uti_publication != ''}
										<a href="../PdfCltChptDetail.php" Target="_blank"><img height="30" src="../img/pdf.png"
												title="{#Detail_par_equipe#} - public" /></a>
									{/if}
								</td>
							</tr>
							<tr>
								<td align='left'>
									<a href="FeuilleCltNiveauJournee.php" Target="_blank"><img height="30" src="../img/pdf.png"
											title="{#Detail_par_journee#} - admin" /></a>
								</td>
						...
						{else}
							...
							<tr>
								<td align='left'>
									<a href="FeuilleCltNiveau.php" Target="_blank"><img height="30" src="../img/pdf.png"
											title="{#Classement_general#} - admin" /></a>
								</td>
								<td colspan=2 align='center'>{#Classement_general#}</td>
								<td align='right'>
									{if $Code_uti_publication != ''}
										<a href="../PdfCltNiveau.php" Target="_blank"><img height="30" src="../img/pdf.png"
												title="{#Classement_general#} - public" /></a>
									{/if}
								</td>
							</tr>
							<tr>
								<td align='left'>
									<a href="FeuilleCltNiveauPhase.php" Target="_blank"><img height="30" src="../img/pdf.png"
											title="{#Deroulement#} - admin" /></a>
								</td>
								<td colspan=2 align='center'>{#Deroulement#}</td>
								<td align='right'>
									{if $Code_uti_publication != ''}
										<a href="../PdfCltNiveauPhase.php" Target="_blank"><img height="30" src="../img/pdf.png"
												title="{#Deroulement#} - public" /></a>
									{/if}
								</td>
							</tr>
							{*                            <tr>
...
*} <tr>
								<td align='left'>
									<a href="FeuilleCltNiveauDetail.php" Target="_blank"><img height="30" src="../img/pdf.png"
											title="{#Detail_par_equipe#} - admin" /></a>
								</td>
								<td colspan=2 align='center'>{#Detail_par_equipe#}</td>
								<td align='right'>
									{if $Code_uti_publication != ''}
										<a href="../PdfCltNiveauDetail.php" Target="_blank"><img height="30" src="../img/pdf.png"
												title="{#Detail_par_equipe#} - public" /></a>
									{/if}
								</td>
							</tr>
						...
						<tr>
							<td align='left'>
								<a href="FeuilleListeMatchs.php?Compet={$codeCompet}" Target="_blank"><img height="30"
										src="../img/pdf.png" title="{#Matchs#} - admin" /></a>
							</td>
							<td colspan=2 align='center'>{#Matchs#}</td>
							<td align='right'>
								<a href="../PdfListeMatchs.php?Compet={$codeCompet}" Target="_blank"><img height="30" src="../img/pdf.png"
						...
						{if ($profile <= 3) && $AuthModif == 'O'}
							<tr>
								<td align='center' colspan="4">
									<br>
									<input type="button" onclick="depublicationClt();" name="D&eacute;-publier"
										value="-{#Supprimer_classement_public#}-">
								</td>
							</tr>
						{/if}
								...
								<td>
									...
									<select name="codeSaisonTransfert" id="codeSaisonTransfert" onchange="changeSaisonTransfert()">
										{section name=i loop=$arraySaisonTransfert}
											<Option Value="{$arraySaisonTransfert[i].Code}"
												{if $arraySaisonTransfert[i].Code == $codeSaisonTransfert}selected{/if}>
												{$arraySaisonTransfert[i].Code}</Option>
										{/section}
									</select>
								</td>
							</tr>
								...
								<td>
									...
									<select name="codeCompetTransfert" id="codeCompetTransfert">
										{section name=i loop=$arrayCompetitionTransfert}
											...
											-->
											<Option Value="{$arrayCompetitionTransfert[i][0]}"
												{if $arrayCompetitionTransfert[i][0] == $codeCompetTransfert}selected{/if}>
												{$arrayCompetitionTransfert[i][1]}</Option>
										{/section}
									</select>
								</td>
							</tr>
