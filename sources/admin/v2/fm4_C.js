...
$(function () {
		...
		if (confirm(lang.Valider_score + $('#scoreA').text() + '-' + $('#scoreB').text() + ' ?')) {
			$.post(
				...
				function (data) { // callback
					if (data == 'OK') {
						window.location = current_page + '?idMatch=' + idMatch
					}
					else {
		...
		$.post(
			...
			function (data) { // callback
				try {
					...
					$('tr[id="ligne_' + data.id + '"]').attr('data-code', code_ligne)
					$.post(
						'v2/StatutPeriode.php', // Le fichier cible côté serveur.
						{ // variables
							Id_Match: idMatch,
	...
	$.post(
		...
		function (data) {
			if (data.action == 'start' || data.action == 'run') {
				...
				//		$('#chrono_moins').hide();
				//		$('#chrono_plus').hide();
				$('#heure').css('border-color', '#008800').css('color', '#008800')
				...
				$('#chrono_moins').show()
				$('#chrono_plus').show()
				$('#heure').css('border-color', '#AA0000').css('color', '#AA0000')
	...
	$('#time_plus10').click(function () {
		...
		if (second_2 > 59) { second_2 -= 60; $('#time_plus60').click() }
		if (second_2 < 10) { second_2 = '0' + second_2 }
		$('#time_evt').val(minut_2 + ':' + second_2)
	})
	$('#time_moins10').click(function () {
		...
		if (isNaN(second_2)) { second_2 = 0 }
		second_2 = Number(second_2) - 10
		if (second_2 < 0) { second_2 += 60; $('#time_moins60').click() }
		if (second_2 < 10) { second_2 = '0' + second_2 }
		$('#time_evt').val(minut_2 + ':' + second_2)
	})
	...
	$('#start_button').click(function () {
		...
		//	$('#chrono_moins').hide();
		//	$('#chrono_plus').hide();
		$('#heure').css('border-color', '#00AA00').css('color', '#008800')
	...
	$('#raz_button').click(function () {
		...
		clearInterval(timer)
		Raz()
		$('#heure').css('border-color', '#FFFFFF').css('color', '#000000')
		$.post(
			'v2/setChrono.php',
			{
				idMatch: idMatch,
