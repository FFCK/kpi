 ...
 */
            ...
            $(function() {    
				...
				$('#chargeFeuille').click(function(event){
					...
					window.location = current_page + '?idMatch=' + $('#idFeuille').val();
				});
				/* ORDRE EVTS */
				$('#change_ordre').click(function() {
					if(ordre_actuel == 'up'){
						ordre_actuel = 'down';
						$('#change_ordre span').removeClass('fa-sort-asc').addClass('fa-sort-desc');
						$('#list_evt tbody tr').each(function(){
							$(this).prependTo('#list_evt tbody');
						});
					}else{
						ordre_actuel = 'up';
						$('#change_ordre span').removeClass('fa-sort-desc').addClass('fa-sort-asc');
						$('#list_evt tbody tr').each(function(){
							$(this).prependTo('#list_evt tbody');
						});
					}
