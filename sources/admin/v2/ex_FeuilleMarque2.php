...
{	
	...
	function Load()
	{
		...
		// Compo équipe B
			...
			$sql5 .= "Order By d.Periode DESC, d.Temps DESC, d.Id ";
...
<head>
		...
		<style>
			...
			.note {font-style: italic; text-align: center; color: red;}
			#logo {height: 90px; position: absolute; top: 5px; left: 5px;}
			label {font-weight: bold; }
			#selectionA, #selectionB { vertical-align: top; width: 35%}
			...
			.maxWidth {width: 100%;}
			.joueur {width: 100%;}
			#time_evt, #chrono_ajust, #periode_ajust, #end_match_time {width:50px; text-align: center; background-color: #DFEFFC;}
			#heure {font-size: 2em; width:85px; text-align: center; background-color: #444444; color: #EEEEFF} 
			#time_end_match {width: 60px; text-align: center; background-color: #DFEFFC;}
			#selectionChrono {min-width: 160px;}
			...
			.fm_bouton {
				background: url("images/ui-bg_glass_80_d7ebf9_1x400.png") repeat-x scroll 50% 50% #DFEFFC;
				/*url("images/ui-bg_glass_85_dfeffc_1x400.png") repeat-x scroll 50% 50% #d7ebf9;*/
			...
			#list {border: 0px solid #79B7E7; border-spacing : 0; border-collapse : collapse; cursor: pointer;}
			#list {float: center;}
			#list td {}
			#list img, #listB img {height: 15px;}
			#list tr:hover { border: 1px solid #5795C5; background-color: #FFFF66;}
			#list_header {border: 0px solid #79B7E7; border-spacing : 0; border-collapse : collapse; cursor: pointer;}
			.list_evt_vide {width: 38%; }
			.list_evt_b {width: 2%; text-align: center; border: 1px solid #79B7E7; background-color: white;}
			.list_evt_v {width: 2%; text-align: center; border: 1px solid #79B7E7; background-color: #AAFFAA;}
			.list_evt_j {width: 2%; text-align: center; border: 1px solid #79B7E7; background-color: #FFFFAA;}
			.list_evt_r {width: 2%; text-align: center; border: 1px solid #79B7E7; background-color: #FFAAAA;}
			.list_chrono {width: 12%; text-align: center; border: 1px solid #79B7E7; }
			.list_evt {width: 2%; text-align: center; border: 1px solid #79B7E7;}
			.list_nom {width: 30%; text-align: center; border: 1px solid #79B7E7;}
			...
			.c_but {height: 15px; }
			.icon_parametres {cursor: pointer; background: url("images/ui-icons_2694e8_256x240.png"); background-position: -192px -112px; height: 16px; width: 16px; position: absolute;}
			.icon_rignt {cursor: pointer; background: url("images/ui-icons_2694e8_256x240.png"); background-position: -32px -16px; height: 16px; width: 16px; position: absolute;}
			.score {
			...
			#validScoreMatch i {color: grey;}
			.presentScore {border-radius: 5px; border: 1px solid grey; padding: 8px; margin: 2px; }
			...
			.fm_tabs {width: 210px; position:fixed; top: 5px; right: 5px; padding: 3px; text-align: left; font-size: 1.1em}
			<?php if($verrou != 'O') { ?>
			.editOfficiel, .editStatut, .editNo, .editArbitres, .suppression {color: blue; text-decoration: none;}
			.editOfficiel:hover, .editStatut:hover, .editNo:hover, .editArbitres:hover, .suppression:hover {text-decoration: underline; cursor: pointer; }
			.editNo input {width: 20px; }
			.editNo button {width: 38px; }
			.placehold {font-style: italic; font-size: 0.8em; font-weight: normal;}
			<?php	}	?>
		...
		<script>
			...
			<?php if($readonly != 'O' && $_SESSION['Profile'] > 0 && $_SESSION['Profile'] <= 4) { ?>
			...
			<?php if($verrou != 'O') { ?>
						...
						function(data){ // callback
							if($.isNumeric(data)){
								...
								if(ligne_evt == 'Carton vert') {
									...
									if(nb_cartons >= 3) {
										custom_alert(nb_cartons + 'e <img class="c_carton" src="carton_vert.png" /> pour ce joueur !', 'Attention');
									}
									// Carton d'équipe
				...
				$('#stop_button').hide();
				$('#run_button').hide();
				// si Statut = ON, $.post('getChrono.php', {idMatch : <?php echo $idMatch ?>} ...) : lecture table "chrono" ligne idMatch, adaptation des paramètres du chrono si la ligne existe...
				...
				$('#start_button').click(function(){
					...
					// $.post('setChrono.php', {idMatch : <?php echo $idMatch ?>, action : 'start', } ...) : replace table chrono ligne idMatch...
				});
				$('#stop_button').click(function(){
					...
					// $.post : replace table chrono ligne idMatch...
				});
				...
				$('#raz_button').click(function(){
					...
					// $.post : delete table chrono ligne idMatch...
				});
			<?php	}	?>
	...
	<body>
		<form>
			...
			<div id="avert"></div>
			<a href="#" class="fm_bouton fm_tabs" id="tabs-1_link" title="Paramètres du match">Paramètres du match<span class="icon_parametres"></span></a>
			<a href="#" class="fm_bouton fm_tabs" id="tabs-2_link" title="Déroulement du match">Déroulement du match<span class="icon_rignt"></span></a>
			<h3 class="centre"><?php 
				...
				<div id="accordion">
					...
					<div>
						<div class="moitie">
							...
							<br />
							<span id="typeMatch">
								<input type="radio" name="typeMatch" id="typeMatchClassement" <?php if($row['Type_match'] == 'C') echo 'checked="checked"'; ?> /><label for="typeMatchClassement" title="Egalité possible">match de classement</label>
						...
						<div class="moitie droite">
							...
							<span id="controleMatch">
								<?php if($readonly != 'O' && $_SESSION['Profile'] > 0 && $_SESSION['Profile'] <= 4) { ?>
									<input type="radio" name="controleMatch" id="controleOuvert" <?php if($verrou != 'O') echo 'checked="checked"'; ?> /><label for="controleOuvert" title="Ouvert">Ouvert</label>
								<?php } ?>
								<input type="radio" name="controleMatch" id="controleVerrou" <?php if($verrou == 'O') echo 'checked="checked"'; ?> /><label for="controleVerrou" title="Verrouillé">Verrouillé</label>
							</span>
					...
					<div>
						<div class="moitie">
							...
							<!--<label>Chronomètre temps de jeu : </label><span class="editOfficiel" id="timeshoot"></span><br />-->
							<br />
							...
							<!--<label>Juge de ligne : </label><span class="editOfficiel" id="ligne1"></span><br />
							<label>Juge de ligne : </label><span class="editOfficiel" id="ligne2"></span><br />-->
						</div>
						<div class="moitie droite">
				...
				<table class="maxWidth" id="deroulement_match">
					...
					<tr>
						...
						<td id="selectionChrono" class="centre">
							<div id="zoneChrono">
								Chrono : <input type="text" id="heure" class="fm_input_text" title="Paramètres du chrono" readonly />
							...
							<div id="zoneTemps">
								...
								<br />
								<a href="#" id="valid_evt" class="fm_bouton evtButton2 evtButton3" title="Valider">Valider</a>
