...
class GestionStructure extends MyPageSecure
{
	...
	function AddClub()
	{
		...
		$codeSaison = $affectEquipe === 'POOL' ? 1000 : $myBdd->GetActiveSaison();
	...
	function __construct()
	{
		...
		if (strlen($Cmd) > 0) {
			if ($Cmd == 'AddCD') ($_SESSION['Profile'] <= 2) ? $alertMessage = $this->AddCD() : $alertMessage = 'Vous n avez pas les droits pour cette action.';

			if ($Cmd == 'AddClub') ($_SESSION['Profile'] <= 2) ? $alertMessage = $this->AddClub() : $alertMessage = 'Vous n avez pas les droits pour cette action.';
