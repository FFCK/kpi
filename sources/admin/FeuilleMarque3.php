...
?>
		...
		<head>
			...
			<link href="v2/fmv3.css?v=<?= NUM_VERSION ?>" rel="stylesheet">
					...
					<div id="accordion">
						...
						<div class="tabs2_content" id="tabs2-A">
							<div class="moitie">
								...
								<br />
								ID# <input class="ui-button ui-widget ui-corner-all" type="tel" id="idFeuille" pattern="[0-9]{8,9}" placeholder="Game ID# or number">
							...
							<td id="selectionChrono" class="centre">
								...
								<div id="zoneScoreboard">
									...
									<div>
										...
										<img id="shotclock_moins" class="plusmoins" src="../img/moins1.png" alt="" />
...
-->
			...
			<?php if ($readonly != 'O' && $_SESSION['Profile'] > 0 && $_SESSION['Profile'] <= 6) { ?>
				<script type="text/javascript" src="v2/fm3_B.js?v=<?= NUM_VERSION ?>"></script>
			<?php } ?>

			<?php if ($verrou != 'O') { ?>
				<script type="text/javascript" src="v2/fm3_C.js?v=<?= NUM_VERSION ?>"></script>
			<?php }	?>

			<script type="text/javascript" src="v2/fm3_D.js?v=<?= NUM_VERSION ?>"></script>
			<script>
				$(function() {
					...
					<?php	}	?>
					$('#end_match_time').val('<?= $heure_fin ? substr($heure_fin, 0, 5) : '' ?>');
					if (statutMatch != 'END') {
						$('.endmatch').hide();
					}
