...
class FeuillePresencePhoto extends MyPage
{
  ...
  function __construct()
  {
    ...
    $codeEquipe = utyGetGet('team', $codeEquipe); // App4 uses 'team'
    ...
    for ($j = 1; $j < count($arrayJoueur[$idEquipe]) + 1; $j++) {
      if ($j > 1 && ($j - 1) % 12 === 0) {
        ...
        // Affichage - Bandeau/Logo/Sponsor (rupture)
        if (($arrayCompetition['Bandeau_actif'] ?? '') == 'O' && isset($visuels['bandeau'])) {
          $img = redimImage($visuels['bandeau'], 297, 10, 20, 'C');
          $pdf->Image($img['image'], $img['positionX'], 8, 0, $img['newHauteur']);
          // KPI + Logo
    ...
    $pdf->Output('Referees.pdf', \Mpdf\Output\Destination::INLINE);
  }
}
