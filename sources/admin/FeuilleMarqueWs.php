...
-->
      ...
      <!--
      <script type="text/javascript" src="../lib/dayjs-1.11.1/dayjs.min.js"></script>
      <script type="text/javascript" src="../lib/dayjs-1.11.1/plugin/customParseFormat.js"></script>
      <script>
      ...
      <script>
        ...
        let duree_prolongations = '05' // ICF:'05', FFCK:'03'
        let arret_chrono_sur_but = false

        let ancienne_ligne = 0
        let theInEvent = false
        let ordre_actuel = 'up'
        let idMatch = <?= $idMatch ?>;
        let idEquipeA = <?= $row['Id_equipeA'] ?>;
        let idEquipeB = <?= $row['Id_equipeB'] ?>;
        let typeMatch = "<?= $typeMatch ?>"
        let statutMatch = "<?= $statutMatch ?>"
        let publiMatch = "<?= $publiMatch ?>"
        let periode_en_cours = "<?= $periodeMatch ?>"
        ...
        let timer, chrono, minut_max = 10,
          second_max = '00'
        let start_time = new Date()
        let run_time = new Date()
        let temp_time = new Date()
      </script>
      <script type="text/javascript" src="v2/fm2_wsA.js?v=<?= NUM_VERSION ?>"></script>
      ...
      <?php if ($readonly != 'O' && $_SESSION['Profile'] > 0 && $_SESSION['Profile'] <= 6) { ?>
        <script type="text/javascript" src="v2/fm2_wsB.js?v=<?= NUM_VERSION ?>"></script>
      <?php } ?>

      <?php if ($verrou != 'O') { ?>
        <script type="text/javascript" src="v2/fm2_wsC.js?v=<?= NUM_VERSION ?>"></script>
      <?php }  ?>

      <script type="text/javascript" src="v2/fm2_wsD.js?v=<?= NUM_VERSION ?>"></script>
      <script>
        $(function() {
      ...
      <script type="text/javascript" src="v2/fm2_ws.js?<?= NUM_VERSION ?>"></script>
