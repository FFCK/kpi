  ...
  <div class="min-h-screen flex items-center justify-center px-4">
    <div class="w-full max-w-lg">
      ...
      <div class="text-center mb-8">
        ...
        <p class="mt-2 text-gray-600">{{ t('users.select_mandate.title') }}</p>
      ...
      <div class="space-y-3">
        ...
        <button
          :disabled="loading"
          class="w-full text-left p-4 bg-white rounded-lg shadow hover:shadow-md border-2 border-transparent hover:border-blue-500 transition-all disabled:opacity-50"
          @click="selectBase"
        >
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              <div class="font-semibold text-gray-900">{{ mandate.libelle }}</div>
              <div class="text-sm text-gray-500 mt-0.5">
                {{ profileLabel(mandate.niveau) }}
              </div>
              <div v-if="mandate.filters.seasons?.length || mandate.filters.competitions?.length" class="text-xs text-gray-400 mt-1">
            ...
            <UIcon name="i-heroicons-identification" class="w-8 h-8 text-blue-400" />
          </div>
        </button>
      </div>
