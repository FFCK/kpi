...
*/
	...
	if($TypeUpdate == 'ValidScore'){
		...
		$sql  = "UPDATE gickp_Matchs SET ScoreA = '".$Valeur[0]."', ScoreB = '".$Valeur[1]."' WHERE Id = ".$idMatch;
		$result = mysql_query($sql, $myBdd->m_link) or die ("Erreur UPDATE<br />".$sql);
		echo 'OK';
	}elseif($TypeUpdate == 'ValidScoreDetail'){
		$Valeur = explode('-', $Valeur);
		$sql  = "UPDATE gickp_Matchs SET ScoreDetailA = '".$Valeur[0]."', ScoreDetailB = '".$Valeur[1]."', ScoreA = '".$Valeur[0]."', ScoreB = '".$Valeur[1]."' WHERE Id = ".$idMatch;
		$result = mysql_query($sql, $myBdd->m_link) or die ("Erreur UPDATE<br />".$sql);
		echo 'OK';
	}else{
		$sql  = "UPDATE gickp_Matchs SET ".$TypeUpdate." = '".$Valeur."' WHERE Id = ".$idMatch;
		$result = mysql_query($sql, $myBdd->m_link) or die ("Erreur UPDATE<br />".$sql);
		echo 'OK';
	}
